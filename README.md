<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,14,16,20,24&height=200&section=header&text=Kiran%20Dahal&fontSize=60&fontColor=ffffff&fontAlignY=35&desc=AI%20Engineer%20%7C%20Full%20Stack%20Developer%20%7C%20Problem%20Solver&descAlignY=55&descSize=18&animation=fadeIn" width="100%"/>
</p>

<p align="center">
  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=22&duration=3000&pause=800&color=7C3AED&center=true&vCenter=true&multiline=false&random=false&width=700&lines=Building+Intelligent+Systems+%F0%9F%A7%A0;Full+Stack+%26+AI+Engineer+from+Nepal+%F0%9F%87%B3%F0%9F%87%B5;BSc+Computing+with+AI+%40+London+Met+Univ;Turning+Data+into+Decisions" alt="Typing SVG" />
  </a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/BSc%20Computing%20with%20AI-London%20Metropolitan%20University-7C3AED?style=for-the-badge&logo=google-scholar&logoColor=white"/>
  &nbsp;
  <img src="https://img.shields.io/badge/Islington%20College-Kathmandu%2C%20Nepal-6D28D9?style=for-the-badge&logo=academia&logoColor=white"/>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/📍%20Hattiban%2C%20Lalitpur-Nepal-5B21B6?style=flat-square"/>
  &nbsp;
  <!-- Portfolio badge — add your URL here when ready
  <a href="https://your-portfolio-url.com">
    <img src="https://img.shields.io/badge/Portfolio-Visit%20Site-7C3AED?style=flat-square&logo=vercel&logoColor=white"/>
  </a>
  &nbsp;
  -->
  <a href="https://linkedin.com/in/kiran-dahal63">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=flat-square&logo=linkedin&logoColor=white"/>
  </a>
  &nbsp;
  <a href="mailto:dahal.utsav63@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-dahal.utsav63-EA4335?style=flat-square&logo=gmail&logoColor=white"/>
  </a>
  &nbsp;
  <a href="https://github.com/kirandahal63">
    <img src="https://img.shields.io/badge/GitHub-kirandahal63-181717?style=flat-square&logo=github&logoColor=white"/>
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=kirandahal63&style=flat-square&color=7C3AED&label=Profile+Views"/>
  &nbsp;
  <img src="https://img.shields.io/github/followers/kirandahal63?style=flat-square&color=6D28D9&label=Followers&logo=github"/>
  &nbsp;
  <img src="https://img.shields.io/github/stars/kirandahal63?style=flat-square&color=5B21B6&label=Stars&logo=github"/>
</p>

---

## ◈ About

<img align="right" width="320" src="https://github-readme-stats.vercel.app/api?username=kirandahal63&show_icons=true&theme=midnight-purple&hide_border=true&bg_color=0D1117&title_color=7C3AED&icon_color=A78BFA&text_color=C4B5FD&ring_color=7C3AED&count_private=true" />

I am an **AI & Full Stack Engineer** currently pursuing a **BSc (Hons) in Computing with Artificial Intelligence** at London Metropolitan University via Islington College, Kathmandu. I engineer production-grade systems that bridge the gap between intelligent algorithms and real-world usability.

My engineering philosophy centers on building software that is **performant, scalable, and purposeful** — from data annotation pipelines that accelerate ML training workflows to centralized travel booking platforms designed for underserved markets. I approach every project with a product mindset: understanding the user problem first, then architecting solutions that solve it elegantly.

I have hands-on experience across the full stack — Python, Java, Django, REST APIs, SQL databases — and I am deepening my expertise in **machine learning, NLP, and intelligent data systems**. I believe the most impactful AI is built at the intersection of strong software engineering and domain-specific understanding.

**Open To:**  
`Internship Opportunities` &nbsp; `AI/ML Research Collaborations` &nbsp; `Open Source Contributions` &nbsp; `Graduate Roles (2026+)`

---

## ◈ Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,java,js,html,css,sql&theme=dark&perline=8" />
</p>
<p align="center"><sub><b>Languages</b></sub></p>

<p align="center">
  <img src="https://skillicons.dev/icons?i=django,react,bootstrap&theme=dark&perline=8" />
</p>
<p align="center"><sub><b>Frontend & Frameworks</b></sub></p>

<p align="center">
  <img src="https://skillicons.dev/icons?i=mysql,postgresql,sqlite,mongodb&theme=dark&perline=8" />
</p>
<p align="center"><sub><b>Backend & Databases</b></sub></p>

<p align="center">
  <img src="https://skillicons.dev/icons?i=git,github,vscode,linux,postman,docker&theme=dark&perline=8" />
</p>
<p align="center"><sub><b>Cloud, DevOps & Tooling</b></sub></p>

---

## ◈ AI / ML Expertise

<div align="center">

| Domain | Proficiency | Details |
|---|---|---|
| **Natural Language Processing** | ⬛⬛⬛⬛⬜ Advanced | AWS Academy ML for NLP · Text Classification · Tokenization |
| **Data Engineering** | ⬛⬛⬛⬛⬜ Advanced | AWS Data Engineering Academy · Pipeline Design · ETL |
| **Data Annotation & Labeling** | ⬛⬛⬛⬛⬛ Expert | Custom Annotation Platform (LabelFlow) · Dataset Curation |
| **Data Visualization** | ⬛⬛⬛⬛⬜ Advanced | Power BI · Chart.js · Analytical Dashboards |
| **Machine Learning Fundamentals** | ⬛⬛⬛⬜⬜ Intermediate | Supervised Learning · Feature Engineering · Model Evaluation |
| **REST API Design** | ⬛⬛⬛⬛⬛ Expert | Django REST Framework · JSON · API-first Architecture |

</div>

---

## ◈ Featured Projects

<details>
<summary><b>&nbsp;⬡&nbsp; LabelFlow — AI Data Annotation Platform</b></summary>

<br/>

> A full-stack, web-based data annotation platform purpose-built to accelerate machine learning workflows. LabelFlow enables administrators to manage the complete lifecycle of image dataset labeling — from project creation and dataset ingestion to task assignment, quality review, and export — all within a single, unified interface.

<div align="center">

| Attribute | Detail |
|---|---|
| **Stack** | Python · JavaScript · HTML · CSS · Django · Django REST Framework |
| **Architecture** | MVC · REST API-first · Role-based Access Control |
| **Core Features** | Project Management · Image Upload · Annotation Task Assignment · Submission Review · Dataset Export |
| **ML Integration** | Export labeled datasets in training-ready formats for downstream ML pipelines |
| **Security** | Admin / Annotator role separation · Authenticated task access |
| **Impact** | Eliminates dependency on third-party annotation tools · Enables fully self-hosted ML data pipelines |
| **Repository** | [![GitHub](https://img.shields.io/badge/View%20Repository-181717?style=flat-square&logo=github)](https://github.com/kirandahal63/LabelFlow) |

</div>

LabelFlow was designed to address a genuine pain point in the ML development lifecycle: the absence of an accessible, self-hosted annotation platform for teams that cannot afford or trust commercial alternatives. The system implements a clean separation between administrator and annotator roles, ensuring data governance at every stage. Image datasets are uploaded, chunked into tasks, assigned to annotators, reviewed against quality standards, and finally exported as structured JSON/CSV datasets ready for training.

<br/>
</details>

<details>
<summary><b>&nbsp;⬡&nbsp; Gantavya — Intercity Travel Booking & Management System</b></summary>

<br/>

> A centralized, full-stack web application engineered to modernize Nepal's fragmented and manual intercity travel ticketing ecosystem. Gantavya consolidates booking, scheduling, and management operations into a single, reliable platform — replacing paper-based processes with a digital-first experience for both operators and passengers.

<div align="center">

| Attribute | Detail |
|---|---|
| **Stack** | Java · Apache Tomcat · XAMPP · phpMyAdmin · HTML · CSS · JavaScript |
| **Architecture** | MVC · Servlet-based Backend · Relational Database |
| **Scale** | Multi-route · Multi-operator · Centralized Booking Engine |
| **Testing** | Manual end-to-end testing of core booking and reservation flows |
| **Security** | Session management · Role-based operator and passenger access |
| **Impact** | Addresses Nepal's underserved intercity travel market · Reduces manual ticketing errors |
| **Repository** | [![GitHub](https://img.shields.io/badge/View%20Repository-181717?style=flat-square&logo=github)](https://github.com/kirandahal63/Gantavya) |

</div>

Gantavya was conceived as a direct response to the inefficiencies plaguing Nepal's intercity travel industry — a sector where ticket booking is still largely handled through physical counters, prone to double-booking, lack of transparency, and poor customer experience. The platform provides travel operators with a management dashboard to configure routes, schedules, and seat availability while offering passengers a streamlined booking interface. Manual testing was conducted rigorously across all critical booking flows to validate correctness and stability prior to delivery.

<br/>
</details>

<details>
<summary><b>&nbsp;⬡&nbsp; InternUp — Internship Management System</b></summary>

<br/>

> A dynamic internship lifecycle management platform developed for Islington College to digitize and streamline student placement tracking, internship approvals, and coordinator workflows.

<div align="center">

| Attribute | Detail |
|---|---|
| **Stack** | Java · Apache NetBeans · Apache Tomcat · MySQL |
| **Architecture** | Servlet-based MVC · Relational Database |
| **Users** | Students · Coordinators · External Supervisors |
| **Core Features** | Placement Tracking · Approval Workflows · Progress Monitoring |
| **Impact** | Replaces manual spreadsheet-based tracking at Islington College |
| **Repository** | [![GitHub](https://img.shields.io/badge/View%20Repository-181717?style=flat-square&logo=github)](https://github.com/kirandahal63/InternUp) |

</div>

<br/>
</details>

<details>
<summary><b>&nbsp;⬡&nbsp; CodeSafari — Children's HTML Learning Platform</b></summary>

<br/>

> An interactive, jungle-themed web application that teaches HTML fundamentals to children through guided coding exercises, quizzes, and gamified certification — collaboratively designed and delivered as a team project.

<div align="center">

| Attribute | Detail |
|---|---|
| **Stack** | HTML · CSS · JavaScript |
| **Audience** | Children · Early learners |
| **Features** | Interactive Coding Exercises · Quizzes · Certification System |
| **Design** | Jungle theme · Playful UI · Child-safe UX |
| **Repository** | [![GitHub](https://img.shields.io/badge/View%20Repository-181717?style=flat-square&logo=github)](https://github.com/kirandahal63/CodeSafari) |

</div>

<br/>
</details>

---

## ◈ Experience

### Customer Success Representative &nbsp;·&nbsp; ING Skill Academy
`May 2025 – November 2025`

Served as a customer-facing representative responsible for ensuring a high-quality learning experience for students and prospective learners across ING Skill Academy's programs.

- Addressed student inquiries and provided guidance on course selection, enrollment processes, and platform navigation
- Maintained consistent, professional communication with clients across calls, email, and messaging platforms
- Resolved concerns and escalated complex cases efficiently to ensure high satisfaction rates
- Managed and maintained accurate customer records and interaction logs
- Collaborated cross-functionally with internal teams to surface feedback and improve the overall customer journey

`Communication` `CRM` `Customer Success` `Problem Solving` `Team Collaboration`

---

## ◈ Achievements

<div align="center">

| Recognition | Details |
|---|---|
| 🎓 **BSc Computing with AI** | London Metropolitan University via Islington College · 2024–Present |
| 🏆 **Gantavya — Travel Booking Platform** | End-to-end full-stack system for Nepal's intercity travel market |
| 🤖 **LabelFlow — Annotation Platform** | Self-hosted ML data pipeline tool with full admin/annotator workflow |
| 📊 **AWS Data Engineering** | AWS Academy certified data engineering practitioner |
| 🧠 **AWS NLP Certification** | AWS Academy Machine Learning for Natural Language Processing |
| 💼 **Customer Success @ ING Skill Academy** | Professional industry experience in client success and engagement |

</div>

---

## ◈ Certifications

<div align="center">

**Amazon Web Services**

![AWS NLP](https://img.shields.io/badge/AWS%20Academy-ML%20for%20NLP-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)
&nbsp;
![AWS Data](https://img.shields.io/badge/AWS%20Academy-Data%20Engineering-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)

**Development & Tools**

![VCS](https://img.shields.io/badge/Certificate-Code%20Management%20with%20VCS-7C3AED?style=for-the-badge&logo=git&logoColor=white)
&nbsp;
![Power BI](https://img.shields.io/badge/Certificate-Data%20Visualization%20Power%20BI-F2C811?style=for-the-badge&logo=powerbi&logoColor=black)

**Programming**

![Java OOP](https://img.shields.io/badge/Certificate-Java%20OOP-ED8B00?style=for-the-badge&logo=java&logoColor=white)
&nbsp;
![Python](https://img.shields.io/badge/Certificate-Python%20for%20Non--Programmers-3776AB?style=for-the-badge&logo=python&logoColor=white)

</div>

---

## ◈ Coding Profiles

<p align="center">
  <a href="https://leetcode.com/kirandahal63">
    <img src="https://img.shields.io/badge/LeetCode-kirandahal63-FFA116?style=for-the-badge&logo=leetcode&logoColor=black"/>
  </a>
  &nbsp;
  <a href="https://www.geeksforgeeks.org/user/kirandahal63">
    <img src="https://img.shields.io/badge/GeeksforGeeks-kirandahal63-2F8D46?style=for-the-badge&logo=geeksforgeeks&logoColor=white"/>
  </a>
  &nbsp;
  <a href="https://www.hackerrank.com/kirandahal63">
    <img src="https://img.shields.io/badge/HackerRank-kirandahal63-00EA64?style=for-the-badge&logo=hackerrank&logoColor=black"/>
  </a>
</p>

---

## ◈ GitHub Analytics

<p align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=kirandahal63&show_icons=true&theme=midnight-purple&hide_border=true&bg_color=0D1117&title_color=7C3AED&icon_color=A78BFA&text_color=C4B5FD&ring_color=7C3AED&count_private=true"/>
  &nbsp;
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=kirandahal63&layout=compact&theme=midnight-purple&hide_border=true&bg_color=0D1117&title_color=7C3AED&text_color=C4B5FD&langs_count=8"/>
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com?user=kirandahal63&theme=midnight-purple&hide_border=true&background=0D1117&ring=7C3AED&fire=A78BFA&currStreakLabel=C4B5FD&sideLabels=C4B5FD&sideNums=7C3AED&currStreakNum=A78BFA&dates=6D28D9" width="60%"/>
</p>

---

## ◈ GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=kirandahal63&theme=darkhub&no-frame=true&no-bg=true&column=7&margin-w=8&margin-h=8" width="100%"/>
</p>

---

## ◈ Contribution Activity

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=kirandahal63&bg_color=0D1117&color=A78BFA&line=7C3AED&point=C4B5FD&area=true&area_color=3B0764&hide_border=true&radius=6" width="100%"/>
</p>

---

## ◈ Contribution Snake

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/kirandahal63/kirandahal63/output/github-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/kirandahal63/kirandahal63/output/github-snake.svg" />
    <img alt="GitHub contribution snake animation" src="https://raw.githubusercontent.com/kirandahal63/kirandahal63/output/github-snake-dark.svg" width="100%" />
  </picture>
</p>

---

## ◈ Current Focus

```yaml
current_focus:
  learning:
    - Deep Learning & Neural Network Architectures
    - Machine Learning Engineering (MLOps)
    - Advanced NLP — Transformers & LLMs
    - Cloud-native Architecture (AWS)

  building:
    - AI-powered annotation and data pipeline tools
    - Scalable REST API backends with Django
    - Full-stack applications with real-world impact

  exploring:
    - Retrieval-Augmented Generation (RAG)
    - Vector Databases & Semantic Search
    - Ethical AI & Responsible ML Systems

  open_to:
    - AI/ML Engineering Internships
    - Full Stack Development Roles
    - Research Collaborations
    - Open Source Contributions
```

---

## ◈ Connect

<p align="center">
  <a href="mailto:dahal.utsav63@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-dahal.utsav63%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  &nbsp;
  <a href="https://linkedin.com/in/kiran-dahal63">
    <img src="https://img.shields.io/badge/LinkedIn-Kiran%20Dahal-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  &nbsp;
  <a href="https://github.com/kirandahal63">
    <img src="https://img.shields.io/badge/GitHub-kirandahal63-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
  <!-- Portfolio button — uncomment and add URL when ready
  &nbsp;
  <a href="https://your-portfolio-url.com">
    <img src="https://img.shields.io/badge/Portfolio-your--site-7C3AED?style=for-the-badge&logo=vercel&logoColor=white"/>
  </a>
  -->
</p>

---

<p align="center">
  <i>"Engineering is not about writing code — it's about solving the right problems, the right way."</i>
</p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=12,14,16,20,24&height=120&section=footer&animation=fadeIn" width="100%"/>
</p>
